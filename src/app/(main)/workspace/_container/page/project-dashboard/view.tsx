...
const iconButton = tv({
  base:
    "inline-flex h-9 w-9 items-center justify-center rounded-full border border-gray-200 bg-white text-gray-500 hover:bg-gray-50 hover:text-gray-700",
});

const statCard = tv({
  base:
    "relative rounded-2xl border p-4 md:p-6 transition-colors bg-white",
  variants: {
    tone: {
...
const table = tv({
  base:
    "w-full text-sm text-gray-700 border-separate border-spacing-0 rounded-xl overflow-hidden",
});

const th = tv({
  base:
    "sticky top-0 z-10 bg-white/70 backdrop-blur border-b border-gray-200 px-4 py-3 text-left font-medium text-gray-500",
});
...
const pill = tv({
  base:
    "inline-flex min-w-[60px] items-center justify-center rounded-full px-2.5 py-1 text-xs font-semibold",
  variants: {
    tone: {
...
export default function ProjectDashboardPage() {
  const totalPoints = useMemo(
    () => summary.reduce((acc, s) => acc + s.points, 0),
    []
  );
    ...
    <div className="min-h-dvh bg-gray-50">
      ...
      <main className="mx-auto max-w-6xl px-4 py-6 md:py-8">
        ...
        <section className="space-y-3">
          <h2 className="text-sm font-semibold text-gray-900">Total Story Points <span className="text-gray-400">({totalPoints})</span></h2>
              ...
              <tbody className="bg-white">
                {rowsA.map((r, idx) => (
                  <tr key={r.task} className="hover:bg-gray-50/60">
                    <td className={td()}>
                      <div className="flex items-center gap-2">
                        <input type="checkbox" defaultChecked={idx === 2} className="h-4 w-4 rounded border-gray-300 text-emerald-600 focus:ring-emerald-500" />
                        {idx === 2 && (
