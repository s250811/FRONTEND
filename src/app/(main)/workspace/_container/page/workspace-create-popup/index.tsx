 ...
 * - Self‑contained demo page: includes a mocked sidebar backdrop + modal
 * - Drop this into any Client Component file (e.g., app/workspace/new/page.tsx)
 */
...
const formGrid = tv({
    base: 'grid grid-cols-[140px_1fr] gap-x-6 gap-y-6',
});

const labelStyle = tv({
    base: 'col-span-1 pt-2 text-[13px] text-gray-700 flex items-start gap-1',
});

const requiredMark = tv({
    base: 'text-rose-500',
});
...
const textArea = tv({
    base: [
        'block w-full min-h-[112px] rounded-xl border text-[13px] p-4',
        'bg-emerald-50/30 placeholder:text-gray-400',
        'border-emerald-100 focus:border-emerald-600 focus:ring-2 focus:ring-emerald-600/20',
    ].join(' '),
});
...
// Utility: clamp length
const clampLen = (s: string, max: number) => (s.length > max ? s.slice(0, max) : s);

export default function WorkspaceCreateModalDemo() {
    // Form state
    ...
    const onSubmit: React.FormEventHandler<HTMLFormElement> = e => {
        ...
        const payload = {
            ...
            avatar: avatarUrl, // base64 for demo
        };
    ...
    return (
        <div className="min-h-dvh bg-gray-50">
            {/* Mocked App Layout Behind the Modal */}
            <div className="grid grid-cols-[260px_1fr] min-h-dvh">
                {/* Sidebar */}
                <aside className="border-r bg-white p-4 space-y-3">
                    <div className="flex items-center gap-2 text-gray-800 font-semibold">
                        <span className="inline-flex h-6 w-6 items-center justify-center rounded bg-gray-100">Ⅱ</span>
                        프로젝트 명
                    </div>
                    <nav className="text-sm text-gray-700 space-y-1 pt-2">
                        <a className="block rounded px-2 py-1 hover:bg-gray-50">알림함</a>
                        {[1, 2, 3, 4, 5, 6].map(n => (
                            <button key={n} className="block w-full text-left rounded px-2 py-1 hover:bg-gray-50">
                                하위 프로젝트 {n}
                            </button>
                        ))}
                    </nav>
                    <div className="mt-auto text-xs text-gray-400">로그아웃</div>
                </aside>

                {/* Main (blurred by overlay) */}
                <main className="relative">
                    <div className="p-8">
                        <h1 className="text-2xl font-bold">상위 프로젝트 이름</h1>
                        <p className="mt-2 text-sm text-gray-500">여기에 프로젝트 컨텐츠가 들어갑니다…</p>
                    </div>
                </main>
            </div>
            ...
            <div className={overlay()}>
                <div className={modal()} role="dialog" aria-modal="true" aria-labelledby="ws-modal-title">
