...
import type { NotificationsVM } from '../_data/types';
// 서버 액션이 필요하면 아래를 이벤트 핸들러에서 호출
// import { markReadAction, removeNotificationsAction } from '../_flow/route.actions';
...
const toolbar = tv({ base: 'flex items-center justify-between border-b border-gray-200 pb-3' });
const actionBtn = tv({
...
const sectionTitle = tv({ base: 'text-sm text-gray-400 mt-5 mb-2' });
const item = tv({
    base: ['w-full rounded-xl border border-transparent bg-white', 'transition-colors'].join(' '),
    variants: {
        selected: { true: 'bg-emerald-50/70 ring-1 ring-emerald-200', false: 'hover:bg-gray-50' },
    },
});
const titleRow = tv({ base: 'flex items-start gap-2 sm:gap-3' });
const dot = tv({
    base: 'ml-2 mt-1 h-2 w-2 rounded-full',
    variants: { unread: { true: 'bg-red-500', false: 'bg-transparent' } },
});
const bullet = tv({ base: 'list-disc pl-5 text-gray-700 text-[15px] leading-6' });
const badge = tv({
...
/* -------------------------------- View ---------------------------------- */
export default function View(vm: NotificationsVM) {
    const [activeTab, setActiveTab] = useState<'all' | 'assigned' | 'mentioned' | 'unread'>('all');
    const [selectedIds, setSelectedIds] = useState<string[]>([]);

    // VM → 가시 목록
    const allVisible = useMemo(() => {
        const rows = [...vm.groups.Today, ...vm.groups.Yesterday];
        switch (activeTab) {
            case 'assigned':
                return rows.filter(t => t.tags.some(tag => tag.label.includes('할당')));
            case 'mentioned':
                return rows.filter(t => t.tags.some(tag => tag.label.includes('멘션')));
            case 'unread':
                return rows.filter(t => t.unread);
            default:
                return rows;
        }
    }, [vm, activeTab]);
    ...
    const toggleAll = () => setSelectedIds(() => (isAllChecked ? [] : allVisible.map(t => t.id)));
    const toggleOne = (id: string) =>
        setSelectedIds(prev => (prev.includes(id) ? prev.filter(x => x !== id) : [...prev, id]));

    const markRead = async () => {
        // await markReadAction(selectedIds);
        alert(`${selectedIds.length}개 항목을 읽음 처리합니다.`);
    };
    const removeItems = async () => {
        // await removeNotificationsAction(selectedIds);
