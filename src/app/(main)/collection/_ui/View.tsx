...
import type { TrashListVM } from '../_data/types';
// 서버 액션과의 연결을 원할 때 아래 import 사용 (지금은 클라이언트 상태로 동작)
// import { restoreTrashAction, deleteTrashAction } from '../_flow/route.actions';

/* ============================== styles(tv) =============================== */
const pageWrap = tv({ base: 'mx-auto w-full max-w-6xl px-4 py-8' });

const searchBox = tv({ base: 'relative ml-auto w-[320px] max-w-full' });
...
const td = tv({ base: 'px-4 py-4 align-middle border-b border-gray-100' });
const row = tv({
    base: 'transition-colors',
    variants: {
        selected: { true: 'bg-emerald-50/40 ring-1 ring-emerald-200', false: 'hover:bg-gray-50/60' },
        withOutline: { true: 'outline outline-1 outline-emerald-300/70 rounded-xl', false: '' },
    },
});
const tag = tv({
...
/* ============================== View =============================== */
export default function View(vm: TrashListVM) {
    // 초기 데이터는 서버에서 주입
    const [rows, setRows] = useState(vm.rows);
    ...
    const restoreOne = async (id: string) => {
        // await restoreTrashAction([id]); // 서버 액션 연동 시
    ...
    const bulkRestore = async () => {
        // await restoreTrashAction(selectedIds);
        setRows((prev: any) => prev.filter((r: any) => !selected[r.id]));
        setSelected({});
    };
    const bulkDelete = async () => {
        // await deleteTrashAction(selectedIds);
        setRows((prev: any) => prev.filter((r: any) => !selected[r.id]));
        setSelected({});
    };
